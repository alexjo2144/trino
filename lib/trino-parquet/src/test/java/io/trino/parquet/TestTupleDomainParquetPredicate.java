...
public class TestTupleDomainParquetPredicate
{
    ...
    public Object[][] timestampPrecision()
    {
        ...
        return new Object[][] {
                {LogicalTypeAnnotation.TimeUnit.MILLIS, 3, baseTime, baseTime.atZone(ZoneOffset.UTC).toInstant().toEpochMilli() * MICROSECONDS_PER_MILLISECOND},
                // note the rounding of micros
                {LogicalTypeAnnotation.TimeUnit.MICROS, 6, baseTime, baseTime.atZone(ZoneOffset.UTC).toInstant().getEpochSecond() * MICROSECONDS_PER_SECOND + 123457},
                {LogicalTypeAnnotation.TimeUnit.NANOS, 9, baseTime, longTimestamp(9, baseTime)}
        };
    }

    @Test(dataProvider = "timestampPrecision")
    public void testTimestampInt96(LogicalTypeAnnotation.TimeUnit timeUnit, int precision, LocalDateTime baseTime, Object baseDomainValue)
            throws ParquetCorruptionException
    {
    ...
    @Test(dataProvider = "timestampPrecision")
    public void testTimestampInt64(LogicalTypeAnnotation.TimeUnit timeUnit, int precision, LocalDateTime baseTime, Object baseDomainValue)
            throws ParquetCorruptionException
    {
        ...
        // Might be a better method to call here
        long minValue = (baseTime.toEpochSecond(ZoneOffset.UTC) * (long) Math.pow(10, precision)) + Math.round(1.0 * baseTime.getNano() / (long) Math.pow(10, 9 - precision));
        long maxValue = (maxTime.toEpochSecond(ZoneOffset.UTC) * (long) Math.pow(10, precision)) + Math.round(1.0 * maxTime.getNano() / (long) Math.pow(10, 9 - precision));
